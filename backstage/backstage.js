...
const MAX_LENGTH_IN_NEXT_ITEM = 20;

const defaultSlideHeaderText = "";
...
window.OpenLP = {
  animateDOMTextTransition: function (DOMText_1, DOMText_2, useSecondDOM, differentiateAnimation = false) {
    ...
    console.log(useSecondDOM, [hiddenIdx, shownIdx]);
  ...
  getNextItemText: function (allSlides) {
    if (OpenLP.currentSlide + 1 >= allSlides.length) {
      return ""
    }

    const nextSlide = allSlides[OpenLP.currentSlide+1]
    
    $("#interim-next").html(nextSlide.html);
    removeDOMWithPrevOrNextBr($("#interim-next").find(".slide-header-texts"))
    
    let DOMCongregationInstructionTexts = $("#interim-next").find(".congregation-instruction-texts")
    let congregationInstructionText = ""
    if (DOMCongregationInstructionTexts.length > 0) {
      congregationInstructionText = DOMCongregationInstructionTexts[0].innerText.trim();

      removeDOMWithPrevOrNextBr(DOMCongregationInstructionTexts);
    }
    if (congregationInstructionText.length > MAX_LENGTH_IN_NEXT_ITEM) {
      congregationInstructionText = congregationInstructionText.slice(0, MAX_LENGTH_IN_NEXT_ITEM+1) + "..";
    }
    
    let mainText = $("#interim-next")[0].innerText.trim();
    fewFirstWords = mainText.split(" ").slice(0, 3).join(" ");
    if (fewFirstWords.length > MAX_LENGTH_IN_NEXT_ITEM) {
      fewFirstWords = fewFirstWords.slice(0, MAX_LENGTH_IN_NEXT_ITEM+1);
    }

    const allTexts = [congregationInstructionText, fewFirstWords]
    console.log(allTexts);
    if (congregationInstructionText.length == 0) {
      allTexts.splice(0,1);
    }
    if (fewFirstWords.length == 0) {
      allTexts.splice(-1,1);
    }
    console.log(allTexts);
    let finalResult = allTexts.join(" - ");;
  ...
  loadSlide: function (event) {
    $.getJSON(
      ...
      function (data, status) {
        ...
        const DOMNextItemText_1 = $("#next-item-text-1");
        const DOMNextItemText_2 = $("#next-item-text-2");
        ...
        let DOMNextItemText = DOMNextItemText_1;
        if (isUsingSecondDOMGroup) {
          ...
          DOMNextItemText = DOMNextItemText_2;
          activeIdx = 1;
        }
        ...
        nextItemText = OpenLP.getNextItemText(data.results.slides);

        // Set HTML values
        DOMCongInstText.html(congregationInstructionText);
        if (slideHeaderText.startsWith(BLANK)) {
          DOMSlideHeaderText.html("");
        } else {
          DOMSlideHeaderText.html(slideHeaderText);
        }
        DOMNextItemText.html(nextItemText);
        ...
        OpenLP.animateDOMTextTransition(DOMNextItemText_1, DOMNextItemText_2, isUsingSecondDOMGroup);
