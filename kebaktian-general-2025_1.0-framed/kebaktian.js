  ...
  loadSlide: function (event) {
    $.getJSON(
      ...
      function (data, status) {
        ...
        let lowerThirdFadeInDuration = OpenLP.transitionDuration / 12;
        let lowerThirdFadeOutDuration = OpenLP.transitionDuration / 2;
        if (slideHeaderText == "" || mainText.startsWith(BUMPER) || mainText.startsWith(NOT_ANGKA_FULL)) {
          $("#slide-header").fadeOut(lowerThirdFadeOutDuration)
        } else {
          $("#slide-header").fadeIn(lowerThirdFadeInDuration)
        }

        // Hide and show main-text's congregation instruction
        if (congregationInstructionText == "" || mainText.startsWith(BUMPER) || mainText.startsWith(NOT_ANGKA_FULL)) {
          $("#congregation-instruction").fadeOut(lowerThirdFadeOutDuration)
        } else {
          $("#congregation-instruction").fadeIn(lowerThirdFadeInDuration)
        }
